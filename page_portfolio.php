...
function prettyPhoto_scripts() {	
    wp_enqueue_script('jquery-1', CHILD_URL.'/plugins/prettyPhoto/js/jquery-1.4.4.min.js');
    wp_enqueue_style('prettyPhoto', CHILD_URL.'/plugins/prettyPhoto/css/prettyPhoto.css');
    wp_enqueue_script('prettyPhoto2', CHILD_URL.'/plugins/prettyPhoto/js/jquery.prettyPhoto.js');
}
...
function legacy_portfolio_do_post_content() {
    ...
    if ( genesis_get_option('legacy_portfolio_content') == 'excerpts' && ! is_page() ) {
        the_excerpt();
    
    } else {
        if ( genesis_get_option('legacy_portfolio_content_archive_limit') && ! is_page() )
